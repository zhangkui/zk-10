...
public interface BatchFlowMapper extends BaseMapper<BatchFlow> {
    ...
    @Select("SELECT bf.id, bf.batch_id, bf.from_node_id, bf.to_node_id, " +
            "bf.flow_quantity AS quantity, bf.operator_id, bf.operate_time AS operateTime, " +
            "bf.temperature AS transportTemperature, bf.transport_duration AS transportDuration, " +
    ...
    @Select("SELECT bf.*, " +
            "fn.node_name AS sourceNodeName, fn.node_code AS from_node_code, fn.node_type AS from_node_type, " +
            "tn.node_name AS targetNodeName, tn.node_code AS to_node_code, tn.node_type AS to_node_type " +
            "FROM batch_flow bf " +
            "LEFT JOIN cold_chain_node fn ON bf.from_node_id = fn.id " +
            "LEFT JOIN cold_chain_node tn ON bf.to_node_id = tn.id " +
